function define_kernel(memory_mask)
  ...
  return quote
    function QP_solve!(sol, info_, iters, n, m, P_, q_, A_, l_, u_, iwork_, fwork_)
      ...
      #iperm = alloc_mem_sf!(iwork, n + m, 0)
      #iperm_iwork = alloc_mem_sf!(iwork, n + m, 0)
      #AMDPKG.mergeargsort!(iperm, perm, 1, length(perm), iperm_iwork)
      #free_mem_sf!(iwork, n + m, 0)
      #H_perm = (alloc_mem_sf!(iwork, n + m + 1, 0), alloc_mem_sf!(iwork, Hnnz, 0), alloc_mem_sf!(fwork, Hnnz, 0))
      #permute_iwork = alloc_mem_sf!(iwork, 2 * (n + m), 0)
      #AMDPKG.permute_mat(H_perm, H, perm, iperm, permute_iwork)
      #free_mem_sf!(iwork, 2 * (n + m), 0)
      #H = H_perm
      ...
      temp2 = alloc_mem_sf!(fwork, n + m, $mem_temp)
